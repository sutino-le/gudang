...
class ModelDataBarangKeluar extends Model
{
    ...
    private function _get_datatables_query($tglawal, $tglakhir)
    {
        if ($tglawal == '' && $tglakhir == '') {
            ...
            $this->dt = $this->db->table($this->table)->join('pelanggan', 'idpel=pelid', 'left')->where('tglfaktur >=', $tglawal)->where('tglfaktur <=', $tglakhir);
        }
        $i = 0;
    ...
    public function count_all($tglawal, $tglakhir)
    {
        if ($tglawal == '' && $tglakhir == '') {
            ...
            $tbl_storage = $this->db->table($this->table)->join('pelanggan', 'idpel=pelid', 'left')->where('tglfaktur >=', $tglawal)->where('tglfaktur <=', $tglakhir);
        }
